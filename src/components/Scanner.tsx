    ...
    <div className="px-5 pt-6">
      ...
      <div className="mb-5 animate-fade-up">
        <h2 className="font-display text-[26px] font-extrabold text-gray-900 leading-tight tracking-tight">
          Scan a book,<br />
          <span className="text-brand-600">get a fair price.</span>
        </h2>
        <p className="text-sm text-gray-500 mt-1.5 font-medium">
          Just show the front cover — we look up the price automatically.
        </p>
      </div>
      ...
      >
        {preview ? (
              ...
              <>
                ...
                <div className="absolute inset-0 bg-black/30 flex items-end justify-center pb-6">
                  <div className="bg-white/95 backdrop-blur rounded-full px-4 py-2 flex items-center gap-2 shadow-card">
                    ...
                    <span className="text-sm font-semibold text-gray-800">Scanning book…</span>
                  </div>
                </div>
              </>
            )}
          ...
          <div className="aspect-[3/4] flex flex-col items-center justify-center p-6 dotted-bg">
            ...
            <p className="font-display font-bold text-gray-900 text-lg">Upload or Capture</p>
            <p className="text-xs text-gray-500 mt-1 text-center max-w-[240px]">
              Show the front cover so we can read the title
            </p>
          </div>
        )}
      </div>
      ...
      {/* Trust indicators — ScrapUncle vibe */}
      {!preview && (
        <div className="mt-6 grid grid-cols-3 gap-3">
          {[
            { icon: '⚡', label: 'Instant', sub: '2s OCR' },
            { icon: '🎯', label: 'Accurate', sub: 'AI-powered' },
            { icon: '💰', label: 'Fair', sub: 'Locality-based' },
          ].map((item) => (
